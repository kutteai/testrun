...
async function getTokenPrice(address, network) {
  ...
  try {
    ...
    console.warn('CoinGecko failed, trying DEX price:', err);
    // Fall through to DEX price
  }
  ...
  try {
    ...
    console.warn('DEX price failed:', err);
  }
...
exports.handler = async (event, _context) => {
  ...
  try {
    ...
    let price = 0;
    let source = 'none';
    let handlerError = null; // Renamed to avoid conflict
