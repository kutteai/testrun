export class DeFiManager {
    ...
    async getYieldOpportunities(network, token) {
    ...
    async addLiquidity(protocol, tokenA, tokenB, amountA, amountB, network, walletAddress, privateKey) {
        try {
    ...
    // Lend tokens
    async lendTokens(protocol, token, amount, network, walletAddress, privateKey) {
        try {
            const protocolInfo = this.protocols.get(protocol);
            if (!protocolInfo) {
                throw new Error('Protocol not found');
    ...
    // Stake tokens
    async stakeTokens(protocol, token, amount, network, walletAddress, privateKey) {
        try {
            const protocolInfo = this.protocols.get(protocol);
            if (!protocolInfo) {
                throw new Error('Protocol not found');
    ...
    getUserPositions(walletAddress) {
        return Array.from(this.positions.values()).filter(position => position.id.includes(walletAddress));
    }
    // Get positions by protocol
    getPositionsByProtocol(protocol) {
        return Array.from(this.positions.values()).filter(position => position.protocol === protocol);
    }
    // Get positions by network
    getPositionsByNetwork(network) {
        return Array.from(this.positions.values()).filter(position => position.network === network);
    }
    ...
    getProtocolContract(protocol, network) {
        const contracts = {
            'uniswap-v3': {
