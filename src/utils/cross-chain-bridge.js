...
import { getGasPrice, estimateGas } from './web3-utils';
...
export class CrossChainBridge {
        ...
        for (const fromChain of chains) {
            for (const toChain of chains) {
                if (fromChain !== toChain) {
                    ...
                    const provider = BRIDGE_PROVIDERS[routeKey];
                    if (provider) {
    ...
    getAvailableRoutes(fromChain, toChain) {
        const routes = [];
        // Direct route
    ...
    async estimateBridgeTransaction(fromChain, toChain, amount, token = 'ETH') {
        ...
        const gasEstimate = await estimateGas(fromAddress, routes[0].bridge.includes('Polygon') ? '0xA0c68C638235ee32657e8f720a23ceC1bFc77C77' : '0x0000000000000000000000000000000000000000', amount, '0x', // data
        fromChain);
        const bestRoute = routes.reduce((best, current) => current.fee < best.fee ? current : best);
        return {
            routes,
    ...
    async executeTransfer(fromChain, toChain, token, amount, recipient) {
        try {
    ...
    async checkBridgeTransactionStatus(txHash, fromChain) {
        try {
    ...
    getBridgeLimits(fromChain, toChain) {
    ...
    getBridgeContract(fromChain, toChain) {
        const routeKey = `${fromChain}-${toChain}`;
        const provider = BRIDGE_PROVIDERS[routeKey];
        return provider?.contract || '0x0000000000000000000000000000000000000000';
    }
    // Get real bridge configuration
    getBridgeConfig(fromChain, toChain, token) {
