...
const TonScreen: React.FC<ScreenProps> = ({ onNavigate }) => {
  ...
  const [tonWallets, setTonWallets] = useState<TonWallet[]>([]);
  const [selectedWallet, setSelectedWallet] = useState<TonWallet | null>(null);
  const [transactions, setTransactions] = useState<TonTransaction[]>([]);
  const [tokens, setTokens] = useState<TonJetton[]>([]);
  ...
  const [selectedWalletType, setSelectedWalletType] = useState<'v3R1' | 'v3R2' | 'v4R2'>('v4R2');
  ...
  const createTonWallet = async () => {
    ...
    setIsLoading(true);
    try {
      const newWallet = tonUtils.generateWallet(newWalletName, selectedNetwork, selectedWalletType);
      const updatedWallets = [...tonWallets, newWallet];
  ...
  const loadWalletData = async (wallet: TonWallet) => {
    try {
      const [balance, accountInfo, txs] = await Promise.all([
        tonUtils.getBalance(wallet.address, wallet.network),
        tonUtils.getAccountInfo(wallet.address, wallet.network),
        tonUtils.getTransactions(wallet.address, wallet.network)
      ]);
      ...
      const updatedWallets = tonWallets.map(w => 
  ...
  const formatUSD = (ton: number) => {
    const tonPrice = 2; // Example price
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-[#180CB2] to-slate-900 text-white flex flex-col">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold">TON</h1>
      ...
      >
        ...
        {tonWallets.length > 0 && (
          <div>
            <h2 className="text-lg font-semibold text-white mb-4">TON Wallets</h2>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        ...
                        <Zap className={`w-5 h-5 ${selectedWallet?.id === wallet.id ? 'text-blue-400' : 'text-white'}`} />
                      </div>
                      <div>
                        <h3 className="font-semibold text-white">{wallet.name}</h3>
                        <p className="text-slate-400 text-sm">{formatAddress(wallet.address)}</p>
                        <p className="text-slate-500 text-xs">{wallet.walletType} • {wallet.network}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-400">Balance</p>
                      <p className="font-semibold text-white">{formatTON(wallet.balance)} TON</p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        {selectedWallet && (
          <div className="space-y-6">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-white">
                        {showBalance ? formatTON(selectedWallet.balance) : '••••••••'}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
              ...
              <p className="text-slate-400 text-sm mt-2">{selectedWallet.walletType} • {selectedWallet.network}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <motion.button
                ...
                className="p-4 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center space-x-2"
              >
                  ...
                  {transactions.slice(0, 5).map((tx) => (
                    <div key={tx.hash} className="p-4 bg-white/5 rounded-xl border border-white/10">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-white">{formatTON(tx.amount)} TON</p>
        ...
        {showCreateWallet && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            ...
            >
              <h3 className="text-lg font-semibold text-white mb-4">Create TON Wallet</h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-blue-500"
              ...
              <div className="flex space-x-3 mt-6">
                ...
                <button
                  onClick={createTonWallet}
                  disabled={isLoading}
                  className="flex-1 p-3 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-lg hover:from-blue-600 hover:to-cyan-700 transition-colors disabled:opacity-50"
                >
