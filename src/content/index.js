...
window.addEventListener('message', async (event) => {
    ...
    if (event.data.source !== 'paycio-wallet-injected')
        return;
...
async function handleSwitchNetwork(params) {
...
// Handle get NFTs
async function handleGetNFTs(params) {
    const { address, network } = params;
    if (!address || !network) {
        return { success: false, error: 'Missing address or network' };
    }
...
async function handleGetTransactionCount(address) {
    try {
...
async function handleEstimateGas(transaction) {
    try {
        const { estimateGas } = await import('../utils/web3-utils');
        const gasLimit = await estimateGas(transaction.from, transaction.to, transaction.value || '0x0', transaction.data || '0x', 'ethereum');
...
async function handleSignTransaction() {
    ...
    try {
        ...
        // Get transaction data from the request (this should come from the dApp)
        if (!request.transaction) {
            throw new Error('Transaction data is required from the dApp');
        }
